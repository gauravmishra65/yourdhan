-- ============================================================
-- YourDhan – Supabase Database Schema  v1.0
-- Apply this in Supabase → SQL Editor → Run
-- ============================================================
...
-- ──────────────────────────────────────────────────────────
-- 1. SESSIONS  (anonymous device identity, no login needed)
-- ──────────────────────────────────────────────────────────
create table if not exists sessions (
  id           uuid primary key default uuid_generate_v4(),
  device_hint  text,                          -- optional browser fingerprint tag
  created_at   timestamptz default now(),
  last_active  timestamptz default now()
);

comment on table sessions is
  'One row per anonymous device. id is stored in browser localStorage as device_id.';
...
comment on table portfolios is
  'Saved portfolio configurations. Each session can have multiple portfolios.';
...
-- ──────────────────────────────────────────────────────────
-- 4. PORTFOLIO_ASSETS  (holdings within a portfolio)
-- ──────────────────────────────────────────────────────────
create table if not exists portfolio_assets (
  ...
  asset_class  text,                         -- 'equity','bond','commodity','reit' etc.
...
comment on table portfolio_assets is
  'Individual asset allocations within a portfolio. weights must sum to 100.';

-- ──────────────────────────────────────────────────────────
-- 5. PORTFOLIO_SNAPSHOTS  (periodic NAV tracking)
-- ──────────────────────────────────────────────────────────
...
create table if not exists watchlist_items (
  ...
  session_id  uuid not null references sessions(id) on delete cascade,
  symbol      text not null,
  note        text,                          -- optional user tag e.g. "buy on dip"
...
create table if not exists price_alerts (
  ...
  current_ref  numeric,                      -- price at time of alert creation
...
-- ──────────────────────────────────────────────────────────
-- 8. STOCK_NOTES  (user annotations on individual stocks)
-- ──────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────
-- 9. SCREENER_PRESETS  (saved screener filter configurations)
-- ──────────────────────────────────────────────────────────
create table if not exists screener_presets (
  id          uuid primary key default uuid_generate_v4(),
  session_id  uuid not null references sessions(id) on delete cascade,
  name        text not null,
  filters     jsonb not null default '{}',   -- arbitrary filter payload
  is_default  boolean default false,
  created_at  timestamptz default now()
);

-- ──────────────────────────────────────────────────────────
-- 10. COMPARE_PORTFOLIOS  (portfolios saved for comparison)
-- ──────────────────────────────────────────────────────────
create table if not exists compare_portfolios (
  id           uuid primary key default uuid_generate_v4(),
  session_id   uuid not null references sessions(id) on delete cascade,
  name         text not null,
  assets_json  jsonb not null,              -- [{symbol, weight}]
  created_at   timestamptz default now()
);
...
-- ROW LEVEL SECURITY
-- ──────────────────────────────────────────────────────────
-- Strategy: anon users can only access rows where session_id matches
-- the value they provide via a postgres custom setting.
-- The app sets "app.current_session_id" before each query.
...
-- Helper function: returns session id set by app
...
-- sessions: any anon user can create; read/update only own
create policy "sessions_insert" on sessions for insert to anon with check (true);
create policy "sessions_select" on sessions for select to anon using (id = current_session_id());
create policy "sessions_update" on sessions for update to anon using (id = current_session_id());

-- user_preferences
create policy "prefs_all" on user_preferences for all to anon
  using (session_id = current_session_id())
  with check (session_id = current_session_id());

-- portfolios
create policy "portfolios_all" on portfolios for all to anon
  using (session_id = current_session_id())
  with check (session_id = current_session_id());

-- portfolio_assets (scoped via portfolio ownership)
create policy "portfolio_assets_all" on portfolio_assets for all to anon
...
-- portfolio_snapshots
create policy "snapshots_all" on portfolio_snapshots for all to anon
  using (portfolio_id in (select id from portfolios where session_id = current_session_id()))
  with check (portfolio_id in (select id from portfolios where session_id = current_session_id()));

-- watchlist_items
create policy "watchlist_all" on watchlist_items for all to anon
  using (session_id = current_session_id())
  with check (session_id = current_session_id());

-- price_alerts
create policy "alerts_all" on price_alerts for all to anon
  using (session_id = current_session_id())
  with check (session_id = current_session_id());

-- stock_notes
create policy "notes_all" on stock_notes for all to anon
  using (session_id = current_session_id())
  with check (session_id = current_session_id());

-- screener_presets
create policy "presets_all" on screener_presets for all to anon
  using (session_id = current_session_id())
  with check (session_id = current_session_id());

-- compare_portfolios
create policy "compare_all" on compare_portfolios for all to anon
  using (session_id = current_session_id())
  with check (session_id = current_session_id());

-- ──────────────────────────────────────────────────────────
-- INDEXES  (perf for common query patterns)
-- ──────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────
-- HELPER RPC: set_session_context
-- Allows app to set the current_session_id before any query
-- ──────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────
-- HELPER RPC: upsert_session
-- Creates session row if new, updates last_active if existing
-- Returns the session id (useful on first visit)
-- ──────────────────────────────────────────────────────────
...
begin
  ...
  -- also set the postgres context for this connection
  perform set_config('app.current_session_id', p_id::text, true);
  return p_id;
end $$;

-- ──────────────────────────────────────────────────────────
-- HELPER VIEW: portfolio_summary
-- Denormalized view for portfolio list UI
-- ──────────────────────────────────────────────────────────
